...
void SchedulerModule::setUp() {
	...
	uint32_t sum_microsseconds = timeout*1000;
	...
	ros::Duration timeout_time_aux(sum_seconds, sum_microsseconds*1000UL );
	...
	reconfiguration_service = scheduler_service_handler.advertiseService("ParameterReconfiguration", &SchedulerModule::reconfigureParameters, this);
	schedule_pub = scheduler_topic_handler.advertise<messages::ReconfigurationCommand>("log_reconfigure", 1000);
	finish_sub = scheduling_finish_handler.subscribe("event", 1000, &SchedulerModule::receiveEvent, this);
...
bool SchedulerModule::reconfigureParameters(services::ParameterReconfiguration::Request &req, services::ParameterReconfiguration::Response &res) {
	std::map<std::string, ModuleParameters>::iterator modules_it;
...

	{
		...
		for(i = 0;i < req.module_names.size();i++) {
			modules_it = connected_modules.find(req.module_names.at(i));
			modules_it->second.setFrequency(req.frequencies.at(i));
			modules_it->second.setRelativeDeadline(static_cast<uint32_t>(req.deadlines.at(i)));
		}
				...
				if(modules_iterator != connected_modules.end()) {
					...
					if(std::get<1>(*(deadlines_it)) >= time_limit) {
						...
						if(!modules_iterator->second.isExecutedInCycle() && !modules_iterator->second.isActive()) {
							...
							//ROS_INFO("modules_iterator->second.setActive(true): ");
							//ROS_INFO("%s is active? %d",name.c_str(), modules_iterator->second.isActive() );
					...
					std::cout << std::endl;
					std::cout << "Module " << std::get<0>(*(deadlines_it)) << " is disconnected! Scheduling next module." << std::endl;
					std::cout << std::endl;
				}
...
void SchedulerModule::coordinateModules() {
	...
	while( i < scheduling_queue.size() && ros::ok()) {
        messages::ReconfigurationCommand reconfig;
        reconfig.source = ros::this_node::getName();
        reconfig.target = scheduling_queue[i];
        reconfig.action = "skip";

        finished_iterator = std::find(finished_modules.begin(), finished_modules.end(), scheduling_queue[i]);
		
		scheduler_record << "Looking for " << scheduling_queue[i] << " in finish message list." << std::endl;
		...
		if(finished_iterator != finished_modules.end()) {
			...
			scheduler_record << scheduling_modules[scheduling_queue[i]].getDiff().sec << "s " << scheduling_modules[scheduling_queue[i]].getDiff().nsec << "ns" << ">=" << timeout_time.sec << "s " << timeout_time.nsec << "ns" << std::endl;
			...
			if(scheduling_modules[scheduling_queue[i]].getDiff() >= timeout_time || ros::Time::now() > scheduling_modules[scheduling_queue[i]].getMaxTime()) {
				...
				scheduler_record << "Time: " << boost::posix_time::to_iso_extended_string(ros::Time::now().toBoost()) << std::endl;
				scheduler_record  << "Max_Time: " << boost::posix_time::to_iso_extended_string(scheduling_modules[scheduling_queue[i]].getMaxTime().toBoost()) << std::endl << std::endl;
				{
			        std::unique_lock< std::mutex > lock( _modules_mutex );
			    }
				...
				scheduler_record << "Module  " << scheduling_queue[i] << " is running" << std::endl;
				i++;
			}
